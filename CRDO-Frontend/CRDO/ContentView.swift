            ...
            VStack(spacing: 24) {
                ...
                ModernSettingsCard {
                    VStack(alignment: .leading, spacing: 16) {
                        SettingsHeader(icon: "target", title: "Daily Goal", subtitle: "Set your daily running target")
                                ...
                                GeometryReader { geometry in
                                    ZStack(alignment: .leading) {
                                        ...
                                        RoundedRectangle(cornerRadius: 8)
                                            ...
                                            .frame(width: geometry.size.width * CGFloat(dailyGoalMinutes) / 60.0, height: 8)
                                            .animation(.easeInOut(duration: 0.3), value: dailyGoalMinutes)
                                    }
                                }
                                .frame(height: 8)
                ...
                ModernSettingsCard {
                    VStack(alignment: .leading, spacing: 16) {
                        SettingsHeader(icon: "bell", title: "Notifications", subtitle: "Stay motivated with reminders")
                        
                        ModernToggle(isOn: $notificationsEnabled) {
                ...
                ModernSettingsCard {
                    VStack(alignment: .leading, spacing: 16) {
                        SettingsHeader(icon: "location", title: "Location Accuracy", subtitle: "Balance accuracy with battery life")
                        
                        ModernSegmentedPicker(
                            selection: $locationAccuracy,
                ...
                ModernSettingsCard {
                    VStack(alignment: .leading, spacing: 16) {
                        SettingsHeader(icon: "folder", title: "Data Management", subtitle: "Export or manage your data")
                        
                        VStack(spacing: 12) {
                            ModernSettingsRow(
                                icon: "square.and.arrow.up",
                                title: "Export Data",
                                subtitle: "Download your run history",
                                iconColor: .blue
                            ) {
            ...
            RoundedRectangle(cornerRadius: 20)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 20)
                        .stroke(
                            LinearGradient(
                                colors: [Color.gold.opacity(0.3), Color.gold.opacity(0.1)],
                                startPoint: .topLeading,
                                endPoint: .bottomTrailing
                            ),
                            lineWidth: 1
        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 12) {
                ZStack {
                    Circle()
                        .fill(LinearGradient(
                            colors: [Color.gold.opacity(0.2), Color.gold.opacity(0.1)],
                            startPoint: .topLeading,
                            endPoint: .bottomTrailing
                        ))
...
struct ModernToggleStyle: ToggleStyle {
    func makeBody(configuration: Configuration) -> some View {
        HStack {
            ...
            ZStack {
                RoundedRectangle(cornerRadius: 20)
                    .fill(configuration.isOn ? Color.gold : Color.gray.opacity(0.3))
            ...
            ForEach(options, id: \.self) { option in
                Button(action: {
                    ...
                    Text(option)
                        ...
                        .background(
                            RoundedRectangle(cornerRadius: 8)
                                .fill(selection == option ? Color.gold : Color.clear)
                        )
                }
                .buttonStyle(PlainButtonStyle())
            }
...
struct ModernSignOutButton: View {
    let action: () -> Void
